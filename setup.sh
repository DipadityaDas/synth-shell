...
installScript()
{
	...
	local edit_text_file_script="$dir/bash-tools/edit_text_file.sh"
	...
	"##  +-----------------------------------+-----------------------------------+\n"\
	"##  |                                                                       |\n"\
	"##  | Copyright (c) 2014-2019, https://github.com/DipadityaDas/synth-shell  |\n"\
	...
	install)
		...
		cat "${dir}/bash-tools/load_config.sh" |\
			sed 's/^#.*$//g;s/[ \t][ \t]*#.*$//g;/^[ \t]*$/d' >> "$script"
		cat "${dir}/bash-tools/color.sh" |\
			sed 's/^#.*$//g;s/[ \t][ \t]*#.*$//g;/^[ \t]*$/d' >> "$script"
		cat "${dir}/bash-tools/shorten_path.sh" |\
			sed 's/^#.*$//g;s/[ \t][ \t]*#.*$//g;/^[ \t]*$/d' >> "$script"
		cat "${dir}/bash-tools/print_utils.sh" |\
			sed 's/^#.*$//g;s/[ \t][ \t]*#.*$//g;/^[ \t]*$/d' >> "$script"
		cat "$source_script" |\
			sed 's/^#.*$//g;s/[ \t][ \t]*#.*$//g;/^[ \t]*$/d' >> "$script"
...
installerSystem()
{
	...
	local dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
	source "$dir/bash-tools/user_io.sh"
...
promptUser()
{
	local dir="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )"
	source "$dir/bash-tools/user_io.sh"
	echo "Installation wizard for synth-shell"
